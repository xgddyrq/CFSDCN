...
def train(epoch, logger):
    ...
    logger.info("===> Epoch {} Complete: Avg. Loss: {:.6f} time: {:.4f}".
...
def test(epoch, logger):
    ...
    print(timeR)
    logger.info('===> Epoch {}: testing psnr = {:.2f}, ssim = {:.3f}, time: {:.4f}'
                .format(epoch, psnr_mean, ssim_mean,(end - begin)))
    model.train()
    return pred, truth, psnr_list, ssim_list, psnr_mean, ssim_mean

def main():
    model.load_state_dict({k.replace('module.', ''): v for k, v in torch.load("/home/yangjincheng/MST/simulation/train_code/exp/new_ndcn_l/2023_12_10_17_21_35/model/model_epoch_484.pth").items()},
                         strict=True)
